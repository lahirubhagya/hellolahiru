...
function initNastik() {
    ...
    //   Isotope------------------
    // Replace the existing Isotope function (function n()) in your scripts.js with this:
    // Dynamic animated filtering with CSS custom properties

    // Replace the function n() in your scripts.js with this:
    // Smooth filtering without blinking
    ...
    function n() {
        if ($(".gallery-items").length) {
            ...
            // Set dynamic animation delays on initial load
            $items.each(function(index) {
                $(this).css('--item-delay', (index * 0.05) + 's');
            });

            // Initial counter setup
            var totalItems = $items.length;
            $(".all-album").html('/' + totalItems);
            $(".num-album").html(totalItems);

            // Add initial class to all items
            $items.addClass('filtered-in');

            // Filter functionality with smooth fade
            $filters.on("click", function (e) {
                ...
                if ($grid.hasClass('filtering')) {
                    return false;
                }

                // Remove active class from all filters
                $filters.removeClass("gallery-filter-active");
                ...
                setTimeout(function() {
                    ...
                    // Hide all items
                    $items.removeClass('filtered-in').addClass('filtered-out');
                    ...
                    if (filterValue === '*') {
                        ...
                        $itemsToShow = $items.filter(filterValue);
                    }

                    // Update counter
                    $(".num-album").html($itemsToShow.length);

                    // Reset animation delays for visible items only
                    $itemsToShow.each(function(index) {
                        $(this).css('--item-delay', (index * 0.05) + 's');
                    });

                    // Show filtered items
                    $itemsToShow.removeClass('filtered-out').addClass('filtered-in');

                    // Trigger reflow for animation
                    void $itemsToShow[0].offsetWidth;
...
//   scroll to fixed ------------------
    ...
    $(".project-details1").scrollToFixed({
        minWidth: 1064,
        zIndex: 5,
        marginTop: 85,
        removeOffsets: true,
        limit: function () {
            var a = $(".limit-box21").offset().top - $(".project-details1").outerHeight(true);
            return a;
        }
    });
